...
- (void)renderPixelBuffer:(CVPixelBufferRef)pixelBuffer
    ...
    int width = (int)CVPixelBufferGetWidth(pixelBuffer);
    int height = (int)CVPixelBufferGetHeight(pixelBuffer);
    CVReturn cvRet = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, _glCache, pixelBuffer, NULL, GL_TEXTURE_2D, GL_RGBA, width, height, GL_BGRA, GL_UNSIGNED_BYTE, 0, &cvTexture);
    if (!cvTexture || kCVReturnSuccess != cvRet) {
        NSLog(@"CVOpenGLESTextureCacheCreateTextureFromImage error %d", cvRet);
    }
    outputTexture = CVOpenGLESTextureGetName(cvTexture);
...
//    }
//    if (ret) {
        [self renderTexture:outputTexture rotate:rotate];
//    }else{
...
- (void)renderTexture:(GLuint)texture mask:(GLuint)maskTexture rotate:(st_rotate_type)rotate {
    if ([EAGLContext setCurrentContext:self.glContext]) {
        [self drawFrameWithTexture:texture mask:maskTexture rotate:rotate];
    }
}

- (BOOL)drawFrameWithTexture:(GLuint)texture mask:(GLuint)maskTexture rotate:(st_rotate_type)rotate {
    static const GLfloat squareVertices[] = {
        -1.0f, -1.0f,
        1.0f, -1.0f,
        -1.0f,  1.0f,
    ...
    if (!_maskShaderProgram) {
        _maskShaderProgram = getShaderProgram(@"Mask", self.glContext);
    }
    glUseProgram(_maskShaderProgram);
    
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, texture);
    glUniform1i(glGetUniformLocation(_maskShaderProgram, "OriginTexture"), 0);

    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, maskTexture);
    glUniform1i(glGetUniformLocation(_maskShaderProgram, "MaskTexture"), 1);
    
    GLuint p = glGetAttribLocation(_maskShaderProgram, "Position");
    GLuint v = glGetAttribLocation(_maskShaderProgram, "OriginTextureCoords");
    GLuint m = glGetAttribLocation(_maskShaderProgram, "MaskTextureCoords");

    glEnableVertexAttribArray(p);
    glVertexAttribPointer(p, 2, GL_FLOAT, 0, 0, squareVertices);
    glEnableVertexAttribArray(v);
    glVertexAttribPointer(v, 2, GL_FLOAT, 0, 0, textureVertices);
    glEnableVertexAttribArray(m);
    glVertexAttribPointer(m, 2, GL_FLOAT, 0, 0, textureVertices);
