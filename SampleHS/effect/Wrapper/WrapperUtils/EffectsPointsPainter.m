...
//    renderPassDescriptor.colorAttachments[0].storeAction = MTLStoreActionStore;
    ...
    if (renderPassDescriptor) {
        ...
        for (int i = 0; i < detectResult.face_count; i ++) {
            ...
            int extPointsCount = detectResult.p_faces[i].extra_face_points_count;
            if (extPointsCount > 0) {
                st_pointf_t points_array[extPointsCount];
                for (int iOfExt = 0; iOfExt < extPointsCount; iOfExt ++) {
                    points_array[iOfExt] = detectResult.p_faces[i].p_extra_face_points[iOfExt];
                }
                [renderCommandEncoder setVertexBytes:&(points_array) length:sizeof(points_array) atIndex:STVertexInputIndexVertices];
                [renderCommandEncoder setVertexBytes:&_viewPortSize length:sizeof(_viewPortSize) atIndex:STVertexInputIndexViewportSize];
                [renderCommandEncoder setRenderPipelineState:_renderPipelineState];
                [renderCommandEncoder drawPrimitives:MTLPrimitiveTypePoint vertexStart:0 vertexCount:extPointsCount];
            }
        }
        ...
        for (int i = 0; i < detectResult.wrist_count; i ++) {
            int currentPointCount = detectResult.p_wrist[i].points_count;
            st_pointf_t points_array[currentPointCount];
            for (int j = 0; j < currentPointCount; j ++) {
                points_array[j] = detectResult.p_wrist[i].p_key_points[j];
            }
            [renderCommandEncoder setVertexBytes:&(points_array) length:sizeof(points_array) atIndex:STVertexInputIndexVertices];
            [renderCommandEncoder setVertexBytes:&_viewPortSize length:sizeof(_viewPortSize) atIndex:STVertexInputIndexViewportSize];
            [renderCommandEncoder setRenderPipelineState:_renderPipelineState];
