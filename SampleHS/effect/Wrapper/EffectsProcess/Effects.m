...
- (instancetype)initWithType:(EffectsType)type cmdQueue:(id <MTLCommandQueue>)cmdQueue {
    self = [super init];
    if (self) {
        [self createHandleWithType:type cmdQueue:cmdQueue];
    }
    return self;
}

- (void)createHandleWithType:(EffectsType)type cmdQueue:(id <MTLCommandQueue>)cmdQueue {
#if METAL_FLAG
    st_handle_t handle;
    st_result_t ret;
    st_result_t setRet = ST_OK;
    
    st_effect_device_info_t device_info = { EFFECT_DEVICE_TYPE_METAL, (__bridge void *)cmdQueue };
    ...
    switch (type) {
        ...
        {
            ret = st_mobile_effect_create_handle_ex(EFFECT_CONFIG_INNER_RENDER_CONTEXT, &device_info, &handle);
            if (ret != ST_OK) {
...
//            self.mglContext = mglContext;
        }
            break;
        case EffectsTypeVideo:
            ret = st_mobile_effect_create_handle_ex(EFFECT_CONFIG_INNER_RENDER_CONTEXT|EFFECT_CONFIG_IMAGE_MODE, &device_info, &handle);
            setRet = st_mobile_effect_set_module_state_change_callback(handle, _modul_state_change_callback);
            if (setRet != ST_OK) {
                NSLog(@"st_mobile_effect_set_module_state_change_callback error %d", setRet);
            }
...
- (st_result_t)getEffectType:(st_effect_beauty_type_t)type mode:(int *)mode {
    st_result_t iRet = ST_OK;
    iRet = st_mobile_effect_get_beauty_mode(self.handle, type, mode);
    if (iRet != ST_OK) {
        NSLog(@"st_mobile_effect_get_beauty_mode %d failed: %d", type, iRet);
    }
    return iRet;
}
...
-(st_result_t)replacePackage:(int)oldPackageId packagePath:(NSString *)packagePath packageId:(int *)packageId {
    return st_mobile_effect_replace_package(self.handle, oldPackageId, packagePath.UTF8String, packageId);
}
